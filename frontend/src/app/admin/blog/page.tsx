    ...
    <div>
      ...
      {showCreate && (
        <div className="card-surface p-6 mb-8">
          <h2 className="font-serif text-2xl text-text-primary mb-6">Create Post</h2>
          <BlogForm form={createForm} setForm={setCreateForm} onSubmit={createPost}
            saving={createSaving} error={createError} submitLabel="Create Post" />
        </div>
      )}
          ...
          <div className="min-h-screen flex items-start justify-center py-8 px-4">
            <div className="w-full max-w-4xl bg-[#111] border border-white/10 p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-serif text-2xl text-text-primary">Edit Post</h2>
                <button onClick={() => setEditingId(null)} className="text-text-muted hover:text-text-primary text-2xl leading-none">&times;</button>
              </div>
              <BlogForm form={editForm} setForm={setEditForm} onSubmit={saveEdit}
                saving={editSaving} error={editError} submitLabel="Save Changes" />
            </div>
          </div>
        </div>
      )}
